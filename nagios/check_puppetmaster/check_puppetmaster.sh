...
#                                                                           #
#############################################################################
# Original Author: George Hansper            e-mail:  george@hansper.id.au  #
#############################################################################
...
function usage () {
    cat <<-EOF
        Usage: $0 [ -h ] [ --help ] [ -H hostname ] [ --hostname hostname ]
                  ...
                  [ -t integer ] [ --timeout integer ] [ -v integer ] [ --version integer ]
                  [ -s string ] [ --ssl_dir string ]
           ...
           -s, --ssl_dir  ... ssl directory (default is $SSLDIR)
...
T1=`date +%s.%N`
CURL_NODE="`curl -sS --max-time $TIMEOUT --insecure --cert $SSLDIR/certs/${HOST}.pem  --key $SSLDIR/private_keys/${HOST}.pem  --cacert $SSLDIR/certs/ca.pem "${URL}" 2>&1`"
CURL_RESULT=$?
T2=`date +%s.%N`
...
if [ "$CURL_RESULT" != 0 ]; then
    ...
    EXIT=1
    MESSAGE="environment not found in https://${PUPPETMASTER}:${PORT}/production/node/${HOST}. Curl output: ${CURL_NODE}"
fi
